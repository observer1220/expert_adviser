...
class QuantTradingBot:
    ...
    def execute_trade(self, price, signal):
        ...
        if self.position == 0 and signal != 0:  # 開倉
            ...
            profit_loss = (
                (price - self.entry_price) * self.lot_size * 100000
                if self.position == 1
                else (self.entry_price - price) * self.lot_size * 100000
            )
            ...
            if price_diff <= -self.stop_loss:  # 止損
                exit_price = (
                    ...
                    if self.position == 1
                    else self.entry_price + self.stop_loss
                )
                profit_loss = (
                    (exit_price - self.entry_price) * self.lot_size * 100000
                    if self.position == 1
                    else (self.entry_price - exit_price) * self.lot_size * 100000
                )
                self.balance += profit_loss
                ...
                exit_price = (
                    ...
                    if self.position == 1
                    else self.entry_price - self.take_profit
                )
                profit_loss = (
                    (exit_price - self.entry_price) * self.lot_size * 100000
                    if self.position == 1
...
if __name__ == "__main__":
    # 讀取 AUDCAD 歷史數據 CSV 文件
    csv_file_path = "AUDCAD_historical_data.csv"  # 替換為你的 CSV 文件路徑
